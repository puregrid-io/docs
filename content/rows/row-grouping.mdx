...
```ts:title=Set+group+to+true
const groupCellRenderer: GroupCellRenderer<Product> = (column, row) => {
...
Another nice touch you might notice in the example is to not render values in cells which are grouped.

```tsx:title=Don't+render+group+values+in+normal+rows
