...
<Grid<YourDataShape>
  ...
  wrapCells
  detailsRowRenderer={(row: DetailsRow<string>) => {
...
`noRows` prop allows you to render something custom when there are no rows such as a loading spinner or a message (or both). It defaults to `null` and any `ReactNode` is permitted.
...
You can toggle on virtual rows so that only rows in the viewport are rendered. Rows must have a fixed height when using this option. See [Virtual Rows](/rows/virtual-rows).
